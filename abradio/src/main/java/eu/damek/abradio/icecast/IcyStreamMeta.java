package eu.damek.abradio.icecast;
...
//			con.setRequestProperty("Accept", null);
	...
	private static Map<String, String> parseMetadata(String metaString) {
		...
		for (String metaPart : metaParts) {
			...
			if (m.find()) {
				metaDataName = m.group(1);
				meta = m.group(2);
			} else {
				Pattern p1 = Pattern.compile("StreamTitle=\\s*(.*)$");
				Matcher m1 = p1.matcher(metaPart);
				if (m1.find()) {
					metaDataName = "StreamTitle";
					meta = m1.group(1);
				}
			}
			if (!meta.equalsIgnoreCase("") || !metaDataName.equalsIgnoreCase("")) {
				String str = "";
				meta = meta.trim();
